<nav class="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200 shadow-sm w-full">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex justify-between items-center">
        ...
        <div class="flex items-center gap-3 sm:gap-6">
            <a href="{{ route('employee.dashboard') }}" class="hidden md:block text-sm transition-colors cursor-pointer {{ ($active ?? '') == 'dashboard' ? 'text-brand-600 font-bold border-b-2 border-brand-500 pb-1' : 'text-slate-500 hover:text-brand-600 font-semibold' }}">{{ __('Dashboard') }}</a>
            <a href="{{ route('employee.requests') }}" class="hidden md:block text-sm transition-colors cursor-pointer {{ ($active ?? '') == 'requests' ? 'text-brand-600 font-bold border-b-2 border-brand-500 pb-1' : 'text-slate-500 hover:text-brand-600 font-semibold' }}">{{ __('Requests') }}</a>
            <a href="{{ route('employee.offers') }}" class="hidden md:block text-sm transition-colors cursor-pointer {{ ($active ?? '') == 'offers' ? 'text-brand-600 font-bold border-b-2 border-brand-500 pb-1' : 'text-slate-500 hover:text-brand-600 font-semibold' }}">{{ __('Offers') }}</a>
            <a href="{{ route('employee.reports') }}" class="hidden md:block text-sm transition-colors cursor-pointer {{ ($active ?? '') == 'reports' ? 'text-brand-600 font-bold border-b-2 border-brand-500 pb-1' : 'text-slate-500 hover:text-brand-600 font-semibold' }}">{{ __('Reports') }}</a>

            {{-- Language Switcher --}}
            <div class="flex items-center gap-2 border-s border-slate-200 ps-3 sm:ps-4 ms-1 sm:ms-2">
                @if(app()->getLocale() === 'en')
                    <a href="{{ route('lang.switch', 'ar') }}" class="text-xs sm:text-sm font-bold text-slate-500 hover:text-brand-600 bg-slate-100 hover:bg-brand-50 px-2 py-1 rounded-md transition-colors" title="Switch to Arabic">عربي</a>
                @else
                    <a href="{{ route('lang.switch', 'en') }}" class="text-xs sm:text-sm font-bold text-slate-500 hover:text-brand-600 bg-slate-100 hover:bg-brand-50 px-2 py-1 rounded-md transition-colors" title="Switch to English">EN</a>
                @endif
            </div>
                ...
                <button @click="open = !open" class="flex items-center gap-2 sm:gap-3 cursor-pointer group hover:bg-slate-50 p-1 sm:p-1.5 sm:pr-4 rounded-full transition-colors border border-transparent hover:border-slate-200 outline-none">
                    ...
                    <div class="hidden sm:flex flex-col text-left">
                        ...
                        <span class="text-xs font-medium text-slate-500 flex items-center gap-1">
                            {{ Auth::user()->employee->diploma ?? __('Caregiver') }}
