<x-layouts.employee active="requests">
    @section('title', __('My Care Requests'))

    @if(Auth::user()->employee->status === 'active')
        <x-employee.page-header 
            breadcrumb="{{ __('Management') }}" 
            title="{{ __('Care Requests') }}" 
            subtitle="{{ __('Review and manage incoming care requests from families.') }}" 
        />
            ...
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 p-5">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-2 relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search by Request ID or Family name...') }}"
                            class="w-full pl-11 rtl:pr-11 rtl:pl-4 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-slate-50 focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium text-sm">
                    </div>
                    ...
                    <div>
                        <select name="status" onchange="this.form.submit()" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-slate-50 focus:bg-white outline-none transition-all font-medium text-sm appearance-none cursor-pointer text-slate-700 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%2394A3B8%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_12px] rtl:bg-[left_16px_center] ltr:bg-[right_16px_center] bg-no-repeat rtl:pl-10 ltr:pr-10">
                            ...
                            <option value="assigned" {{ request('status') == 'assigned' ? 'selected' : '' }}>{{ __('Accepted') }}</option>
                            <option value="rejected" {{ request('status') == 'rejected' ? 'selected' : '' }}>{{ __('Declined') }}</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                        <tr class="bg-slate-50/80 backdrop-blur">
                            <th class="pb-3 pt-4 px-4 sm:px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100 sticky top-0 whitespace-nowrap rtl:text-right">{{ __('Request ID') }}</th>
                            <th class="pb-3 pt-4 px-4 sm:px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100 sticky top-0 whitespace-nowrap rtl:text-right">{{ __('Family Profile') }}</th>
                            <th class="pb-3 pt-4 px-4 sm:px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100 sticky top-0 whitespace-nowrap rtl:text-right">{{ __('Service Type') }}</th>
                            <th class="pb-3 pt-4 px-4 sm:px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100 sticky top-0 whitespace-nowrap rtl:text-right">{{ __('Date Range') }}</th>
                            <th class="pb-3 pt-4 px-4 sm:px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest border-b border-slate-100 sticky top-0 rtl:text-left ltr:text-right whitespace-nowrap">{{ __('Actions/Status') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-4 px-4 sm:px-6 border-b border-slate-50">
                                    <a href="{{ route('employee.family.profile', $req->family->id) }}" class="group">
                                        <div class="font-bold text-sm text-slate-800 group-hover:text-brand-600 transition-colors flex items-center gap-1.5">
                                            {{ $req->family->user->name ?? __('Unknown') }}
                                            <svg class="w-3.5 h-3.5 text-slate-300 group-hover:text-brand-500 transition-colors rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                        </div>
                                    </a>
                                    <div class="flex items-center gap-1.5 mt-0.5">
                                        ...
                                        @if($req->family->user->localization && $req->family->user->localization->latitude && $req->family->user->localization->logitude)
                                            <a href="https://www.google.com/maps/dir/?api=1&destination={{ $req->family->user->localization->latitude }},{{ $req->family->user->localization->logitude }}" target="_blank" rel="noopener noreferrer"
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-brand-50 text-brand-700 rounded-md text-[10px] font-bold hover:bg-brand-100 transition-colors ring-1 ring-inset ring-brand-500/20 whitespace-nowrap" title="{{ __('Get directions') }}">
                                ...
                                <td class="py-4 px-4 sm:px-6 border-b border-slate-50">
                                    <span class="inline-flex px-2.5 py-1 text-[10px] font-bold text-brand-700 bg-brand-50 rounded-lg ring-1 ring-inset ring-brand-500/20">
                                        {{ $req->offre->service_type ?? __('Service Offre') }}
                                    </span>
                                </td>
                                <td class="py-4 px-4 sm:px-6 border-b border-slate-50 text-sm text-slate-600 font-medium">
                                    <div class="flex flex-col gap-0.5">
                                        <span class="whitespace-nowrap"><strong class="text-[10px] font-bold text-slate-400 uppercase rtl:ml-1 ltr:mr-1">{{ __('From') }}</strong> {{ \Carbon\Carbon::parse($req->start_date)->format('M d, Y') }}</span>
                                        <span class="whitespace-nowrap"><strong class="text-[10px] font-bold text-slate-400 uppercase rtl:ml-1 ltr:mr-1">{{ __('To') }}</strong> {{ \Carbon\Carbon::parse($req->end_date)->format('M d, Y') }}</span>
                                    </div>
                                </td>
                                    ...
                                    @if($req->status === 'pending')
                                        <div class="flex flex-col sm:flex-row items-center justify-end gap-2">
                                            ...
                                            <form action="{{ route('employee.requests.reject', $req->id) }}" method="POST" class="w-full sm:w-auto">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="w-full px-4 py-2 bg-white border border-slate-200 hover:bg-rose-50 text-rose-600 font-bold rounded-xl text-sm transition-all shadow-sm">{{ __('Decline') }}</button>
                                            </form>
                                            <form action="{{ route('employee.requests.accept', $req->id) }}" method="POST" class="flex items-center gap-2 w-full sm:w-auto">
                                                ...
                                                <div class="relative flex-1 sm:w-24">
                                                    <input type="number" name="price" required placeholder="{{ __('Price') }}" min="0" step="0.01" class="w-full ltr:pl-3 ltr:pr-8 rtl:pr-3 rtl:pl-8 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-white outline-none text-sm font-medium transition-all">
                                                    <span class="absolute ltr:right-3 rtl:left-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase">DA</span>
                                                </div>
                                                <button type="submit" class="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-sm transition-all shadow-md active:scale-95 whitespace-nowrap">{{ __('Accept') }}</button>
                                            </form>
                                        </div>
                                    @elseif($req->status === 'assigned')
                                        <span class="inline-flex px-3 py-1.5 text-[10px] font-bold text-emerald-700 bg-emerald-50 rounded-lg ring-1 ring-inset ring-emerald-500/20 uppercase tracking-wider">{{ __('Accepted') }}</span>
                                    @else
                                        <span class="inline-flex px-3 py-1.5 text-[10px] font-bold text-rose-700 bg-rose-50 rounded-lg ring-1 ring-inset ring-rose-500/20 uppercase tracking-wider">{{ __('Declined') }}</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
            ...
            function openDiscussionModal(requestId, messages) {
                ...
                if (messages.length === 0) {
                    ...
                    messages.forEach(msg => {
                        ...
                        div.innerHTML = `
                            <div class="max-w-[85%] rounded-2xl px-4 py-3 text-sm ${isMe ? 'bg-brand-600 text-white ltr:rounded-tr-none rtl:rounded-tl-none' : 'bg-white border border-slate-200 text-slate-700 ltr:rounded-tl-none rtl:rounded-tr-none shadow-sm'}">
                                <p class="font-medium leading-relaxed">${msg.message}</p>
                                <span class="text-[10px] mt-1 block opacity-70 ltr:text-right rtl:text-left font-bold uppercase tracking-wider">${new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                            </div>
                        `;
                ...
                messageForm.addEventListener('submit', async function(e) {
                    ...
                    try {
                        ...
                        if(data.success) {
                            ...
                            div.innerHTML = `
                                <div class="max-w-[85%] rounded-2xl px-4 py-3 text-sm bg-brand-600 text-white ltr:rounded-tr-none rtl:rounded-tl-none shadow-sm">
                                    <p class="font-medium leading-relaxed">${msg.message}</p>
                                    <span class="text-[10px] mt-1 block opacity-70 ltr:text-right rtl:text-left font-bold uppercase tracking-wider">${new Date(msg.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                </div>
                            `;
