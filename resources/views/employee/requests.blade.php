...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col w-full overflow-x-hidden">
    ...
    <x-employee.navbar :active="'requests'" />
                                    ...
                                    <td class="py-4 px-4 sm:px-6 border-b border-slate-50 text-right">
                                        ...
                                        @if($req->status === 'pending')
                                            <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                                                ...
                                                <!-- Reject Form -->
                                                <form action="{{ route('employee.requests.reject', $req->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="px-4 py-2 bg-white border border-slate-200 hover:bg-rose-50 text-rose-600 font-bold rounded-xl text-sm transition-all shadow-sm">
                                                        Decline
                                                    </button>
                                                </form>

                                                <!-- Accept Form with Price Input -->
                                                <form action="{{ route('employee.requests.accept', $req->id) }}" method="POST" class="flex items-center gap-2">
                                                    @csrf
                                                    @method('PATCH')
                                                    <div class="relative w-32">
                                                        <input type="number" name="price" required placeholder="Price" min="0" step="0.01" class="w-full pl-3 pr-8 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 outline-none text-sm font-medium">
                                                        <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400">DA</span>
                                                    </div>
                                                    <button type="submit" class="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-sm transition-all shadow-md">
                                                        Set Price & Accept
                                                    </button>
                                                </form>
