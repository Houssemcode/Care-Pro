<x-layouts.employee active="offers">
    @section('title', __('Create Service Offer'))

    <x-employee.page-header 
        breadcrumb="{{ __('Care Services') }}" 
        title="{{ __('Create New Offer') }}" 
        subtitle="{{ __('Define the details of the care services you provide so families can find and book you.') }}" 
    />
        ...
        <form action="{{ route('employee.offres.store') }}" method="POST" class="p-6 sm:p-10 space-y-8">
            ...
            <div>
                ...
                <select name="service_type" required class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-slate-50 focus:bg-white outline-none transition-all font-medium text-sm appearance-none cursor-pointer text-slate-700 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%2394A3B8%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_12px] rtl:bg-[left_16px_center] ltr:bg-[right_16px_center] bg-no-repeat rtl:pl-10 ltr:pr-10">
                    <option value="" disabled selected>{{ __('Select the type of care you offer...') }}</option>
                    <option value="Child Care">{{ __('Child Care / Babysitting') }}</option>
            ...
            <div>
                ...
                <input type="text" name="wilaya" placeholder="{{ __('e.g., Alger, Oran, Constantine') }}" required
            ...
            <div>
                ...
                <input type="text" name="commune" placeholder="{{ __('e.g., Hydra, Ben Aknoun, El Biar') }}" required
                    class="w-full px-4 py-3.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-slate-50 focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium text-sm">
                @error('commune') <p class="text-rose-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
            </div>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                <a href="{{ route('employee.dashboard') }}" class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition-all shadow-sm active:scale-95">{{ __('Cancel') }}</a>
