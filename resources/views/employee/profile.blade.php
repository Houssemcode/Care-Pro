...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col w-full overflow-x-hidden">
    ...
    <x-employee.navbar :active="'profile'" />

    <main class="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 flex flex-col page-enter">
        
        <div class="mb-8">
            <h1 class="text-3xl sm:text-4xl font-display font-extrabold text-slate-900 mb-2">My Profile</h1>
            <p class="text-sm sm:text-base text-slate-500 font-medium">An overview of your professional account and platform statistics.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-1">
                <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    <span class="inline-flex px-3 py-1 text-xs font-bold {{ $config['class'] }} rounded-lg ring-1 ring-inset uppercase tracking-wider mb-6">
                        {{ $config['label'] }}
                    </span>

                    <a href="{{ route('employee.settings') }}" class="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-sm transition-all">
                        Edit Settings
                    </a>
                </div>
            </div>
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-0.5">Completed Jobs</p>
                    ...
                    <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-0.5">Active Offers</p>
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
                    <h3 class="font-display font-bold text-lg text-slate-800 mb-4">Professional Background</h3>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-600 leading-relaxed bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        @if($status === 'active')
                            <span class="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase rounded-lg border border-emerald-100">Verification Complete</span>
                        @else
                            <span class="px-2.5 py-1 bg-amber-50 text-amber-700 text-[10px] font-bold uppercase rounded-lg border border-amber-100">Required for activation</span>
                        @endif
                    </div>
                    ...
                    <form action="{{ route('employee.documents.upload') }}" method="POST" enctype="multipart/form-data" class="mb-8 p-5 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Document Type</label>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">File (PDF, JPG, PNG)</label>
