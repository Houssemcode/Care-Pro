<x-layouts.admin active="profile" title="{{ __('Admin Profile') }}">
    @section('title', __('Admin Profile'))

    <x-admin.page-header 
        breadcrumb="{{ __('Account') }}" 
        title="{{ __('Admin Profile') }}" 
        subtitle="{{ __('Manage your personal admin account.') }}" 
    />
            ...
            <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden">
                <div class="p-8 border-b border-slate-100 flex flex-col sm:flex-row items-center gap-6 bg-slate-50/50">
                    ...
                    <div class="text-center sm:text-left rtl:sm:text-right">
                        ...
                        <p class="text-sm font-semibold text-brand-600 mt-1">{{ __('Super Administrator') }}</p>
                        <p class="text-xs text-slate-400 mt-2 font-medium italic">{{ __('Managed via System Core') }}</p>
                    </div>
                </div>
                ...
                <div class="p-8 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('Full Name') }}</p>
                            <p class="text-base font-bold text-slate-800">{{ Auth::user()->name }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('Email Address') }}</p>
                            <p class="text-base font-bold text-slate-800">{{ Auth::user()->email }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('Phone Number') }}</p>
                            <p class="text-base font-bold text-slate-800">{{ Auth::user()->phone ?? __('Not provided') }}</p>
                        </div>
                        <div class="space-y-1">
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('Member Since') }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{{ __('Families') }}</span>
                        <span class="text-lg font-display font-black text-slate-900">{{ $totalFamilies }}</span>
                    </div>
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{{ __('Caregivers') }}</span>
                        <span class="text-lg font-display font-black text-slate-900">{{ $totalEmployees }}</span>
                    </div>
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{{ __('Active Offers') }}</span>
                        <span class="text-lg font-display font-black text-slate-900">{{ $totalOffres }}</span>
                    </div>
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                        <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{{ __('Total Bookings') }}</span>
                ...
                <div class="mt-8 pt-6 border-t border-slate-100">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-brand-50 text-brand-600 font-bold text-xs hover:bg-brand-600 hover:text-white transition-all group">
                        ...
                        <svg class="w-4 h-4 transform rtl:rotate-180 rtl:group-hover:-translate-x-1 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                    </a>
                </div>
            </div>
        </div>
