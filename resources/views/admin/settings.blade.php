<x-layouts.admin active="settings" title="{{ __('Admin Settings') }}">
    @section('title', __('Admin Settings'))

    <x-admin.page-header 
        breadcrumb="{{ __('Account') }}" 
        title="{{ __('Admin Settings') }}" 
        subtitle="{{ __('Manage your administrator credentials and platform security.') }}" 
    />
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 overflow-hidden">
                <div class="p-6 border-b border-slate-50 bg-slate-50/50">
                    <h3 class="font-display font-bold text-lg text-slate-800">{{ __('Admin Information') }}</h3>
                    <p class="text-xs text-slate-500 font-medium">{{ __('Update your system administrator profile name and email.') }}</p>
                </div>
