<x-layouts.admin active="requests" title="{{ __('Manage Requests') }}">
    @section('title', __('Manage Requests'))

    <x-admin.page-header 
        breadcrumb="{{ __('Requests') }}" 
        title="{{ __('Booking Requests') }}" 
        subtitle="{{ __('Monitor, approve, and finalize prices for bookings.') }}" 
    />
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col w-full">
        <div class="overflow-x-auto custom-scrollbar flex-1 w-full relative min-h-[400px]">
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50 rtl:text-right">{{ __('Request ID') }}</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50 rtl:text-right">{{ __('Family') }}</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50 rtl:text-right">{{ __('Requested Offre') }}</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse ($requests as $req)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-8 py-4">
                                ...
                                <span class="text-slate-400 text-[10px]">{{ __('Start') }}: {{ \Carbon\Carbon::parse($req->start_date)->format('M d, Y') }}</span>
                            </td>
                            <td class="px-8 py-4">
                                <div class="font-semibold text-slate-900">{{ $req->family->user->name ?? __('Unknown Family') }}</div>
                            ...
                            <td class="px-8 py-4">
                                <div class="font-semibold text-brand-600">{{ $req->offre->service_type ?? __('Service Offre') }}</div>
                                <div class="text-[10px] text-slate-500 font-medium">{{ __('Caregiver') }}: {{ $req->offre->employee->user->name ?? __('N/A') }}</div>
                            </td>
                            <td class="px-8 py-4">
                            ...
                            <td class="px-8 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @if($req->status === 'pending')
                                        <button class="px-3 py-1.5 bg-brand-50 hover:bg-brand-100 text-brand-700 font-bold rounded-lg text-xs transition-all border border-brand-100 active:scale-95" title="{{ __('Approve & Set Price') }}"
                                            onclick="openAssignModal(
                                                {{ $req->id }}, 
                                                '{{ addslashes($req->family->user->name ?? __('Family')) }}',
                                                '{{ addslashes($req->offre->employee->user->name ?? __('Caregiver')) }}'
                                            )">
                                            {{ __('Set Price') }}
                                        </button>
                                    @endif
                                    <button class="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg text-xs transition-all active:scale-95" title="{{ __('View Details') }}"
                                        onclick="viewRequestDetails(
                                            '{{ $req->id }}', 
                                            '{{ addslashes($req->offre->service_type ?? __('Service')) }}', 
                                            '{{ addslashes($req->offre->working_house ?? __('N/A')) }}', 
                                            '{{ $req->status }}',
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center py-10 text-slate-400 italic text-sm">{{ __('No booking requests found.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="modal-content">
            <form id="form-assign-caregiver" method="POST" action="">
                ...
                <div class="modal-header">
                    <div>
                        <h3 class="font-display font-bold text-lg text-slate-800">{{ __('Finalize Assignment') }}</h3>
                ...
                <div class="modal-body space-y-5">
                    <div class="bg-brand-50 border border-brand-100 p-4 rounded-xl">
                        <p class="text-[10px] font-bold text-brand-600 uppercase tracking-widest mb-1">{{ __('Requested Caregiver') }}</p>
                        <p class="text-sm font-bold text-brand-900" id="assign-caregiver-name"></p>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">{{ __('Final Price (DZD)') }}</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 rtl:left-auto rtl:right-0 pl-4 rtl:pl-0 rtl:pr-4 flex items-center pointer-events-none text-brand-500 font-bold text-xs">{{ __('DA') }}</div>
                            <input type="number" name="price" required min="0" placeholder="0.00" class="w-full pl-12 rtl:pl-4 rtl:pr-12 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-brand-500 bg-white outline-none font-bold text-slate-800 text-sm">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" onclick="closeModal('modal-assign')" class="px-4 py-2 text-sm font-bold text-slate-500">{{ __('Cancel') }}</button>
                    <button type="submit" class="px-6 py-2 bg-brand-500 text-white font-bold rounded-xl text-sm shadow-lg shadow-brand-500/30">{{ __('Confirm & Assign') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-details" class="modal-overlay z-[100]" onclick="if(event.target===this) closeModal('modal-details')">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="font-display font-bold text-lg text-slate-800">{{ __('Request Details') }}</h3>
            ...
            <div class="modal-body space-y-4">
                <div class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ __('Request ID') }}</span>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="p-4 bg-white border border-slate-100 rounded-xl shadow-sm">
                        <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ __('Service Type') }}</h4>
                        <p class="text-sm font-bold text-slate-800" id="detail-req-service"></p>
                    </div>
                    <div class="p-4 bg-white border border-slate-100 rounded-xl shadow-sm">
                        <h4 class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{{ __('Location') }}</h4>
                ...
                <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-xl shadow-sm" id="detail-contract" style="display: none;">
                    <h4 class="text-[10px] font-bold text-emerald-600 uppercase tracking-wider mb-2">{{ __('Active Contract') }}</h4>
                    <p class="text-xs font-medium text-emerald-800">{{ __('Assigned To') }}: <span class="font-bold" id="detail-contract-emp"></span></p>
                </div>
            </div>
            <div class="modal-footer">
                <form id="form-reject-request" method="POST" action="" class="me-auto" onsubmit="return confirm('{{ __('Reject this request?') }}');">
                    @csrf @method('PATCH')
                    <button type="submit" class="px-4 py-2 text-rose-600 hover:bg-rose-50 font-bold rounded-xl text-xs transition-all border border-transparent hover:border-rose-100" id="btn-reject-req">{{ __('Reject Request') }}</button>
                </form>
                <button onclick="closeModal('modal-details')" class="px-4 py-2 text-sm font-bold text-slate-500">{{ __('Close') }}</button>
            </div>
        </div>
    </div>
    ...
    <script>
        function openAssignModal(reqId, familyName, caregiverName) {
            document.getElementById('assign-req-title').textContent = "{{ __('Request') }} #" + reqId + " - " + familyName;
