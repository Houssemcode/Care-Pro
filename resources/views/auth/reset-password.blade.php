...
<head>
    ...
    <title>{{ __('Reset Password') }} - CarePro</title>
...
<body class="bg-slate-50 antialiased flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden">
        <div class="p-8 sm:p-10">
            <h1 class="text-2xl font-extrabold text-slate-900 mb-2">{{ __('Create New Password') }}</h1>
            <p class="text-sm text-slate-500 font-medium mb-8">{{ __('Please enter and confirm your new secure password.') }}</p>
            ...
            <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">{{ __('New Password') }}</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">{{ __('Confirm Password') }}</label>
