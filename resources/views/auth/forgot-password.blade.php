...
<head>
    ...
    <title>{{ __('Forgot Password?') }} - CarePro</title>
...
<body class="bg-slate-50 antialiased flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md bg-white rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden">
        <div class="p-8 sm:p-10">
            <h1 class="text-2xl font-extrabold text-slate-900 mb-2">{{ __('Forgot Password?') }}</h1>
            <p class="text-sm text-slate-500 font-medium mb-8">{{ __('No problem. Just let us know your email address and we will email you a password reset link.') }}</p>
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('login') }}" class="text-sm font-bold text-slate-500 hover:text-slate-800 transition-colors">{{ __('← Back to login') }}</a>
            </div>
        </div>
    </div>
