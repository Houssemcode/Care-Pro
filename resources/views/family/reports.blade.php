<x-layouts.family active="reports">
    ...
    <x-family.page-header 
        breadcrumb="{{ __('Disputes') }}" 
        title="{{ __('My Reports') }}" 
        subtitle="{{ __('Track the status of disputes or issues involving your account.') }}" 
    />
    ...
    <div class="max-w-4xl space-y-6">
            ...
            <div class="bg-white rounded-[2rem] shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 p-6 flex flex-col relative overflow-hidden transition-all hover:border-rose-200">
                ...
                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-5">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="font-display font-bold text-lg text-slate-800">{{ __($report->report_reason ?? 'Dispute Filed') }}</h3>
                            <p class="text-xs text-slate-500 font-medium mt-0.5">
                                @if($filter === 'sent')
                                    {{ __('Reported Caregiver:') }} <span class="text-slate-800 font-bold uppercase tracking-tight">{{ $report->employee->user->name ?? __('Caregiver ID:') . ' ' . $report->employee_id }}</span> 
                                @else
                                    {{ __('Reported by:') }} <span class="text-slate-800 font-bold uppercase tracking-tight">{{ $report->employee->user->name ?? __('Caregiver ID:') . ' ' . $report->employee_id }}</span> 
                                @endif
                ...
                @if($report->status === 'resolved' && !empty($report->admin_note))
                    <div class="mt-6 pt-5 border-t border-slate-100">
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">{{ __('Response from Support') }}</p>
        ...
        @empty
            <div class="py-20 text-center bg-white rounded-[2rem] border border-slate-100 shadow-sm">
                ...
                <h3 class="font-display font-bold text-xl text-slate-800 mb-2">{{ __('No Reports Found') }}</h3>
