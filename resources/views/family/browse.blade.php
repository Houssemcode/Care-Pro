<x-layouts.family active="browse">
    ...
    <x-family.page-header 
        breadcrumb="{{ __('Search') }}" 
        title="{{ __('Find a Caregiver') }}" 
        subtitle="{{ __('Browse available professionals, filter by your needs, and request a booking.') }}" 
    />
        ...
        <div class="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100 p-5 space-y-4">
            ...
            <div class="flex flex-col md:flex-row gap-4">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Search by name or location...') }}"
            ...
            <div class="flex flex-col md:flex-row gap-3 items-end flex-wrap">
                <div class="w-full md:w-44">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{{ __('Wilaya') }}</label>
                    <select name="wilaya" onchange="this.form.submit()" class="{{ $selectClass }}">
                        <option value="">{{ __('All Wilayas') }}</option>
                ...
                <div class="w-full md:w-44">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{{ __('Arrangement') }}</label>
                    <select name="working_house" onchange="this.form.submit()" class="{{ $selectClass }}">
                        <option value="">{{ __('Any') }}</option>
                        <option value="1" {{ request('working_house') === '1' ? 'selected' : '' }}>{{ __('Live-in') }}</option>
                        <option value="0" {{ request('working_house') === '0' ? 'selected' : '' }}>{{ __('Live-out') }}</option>
                    </select>
                </div>
                ...
                <button type="button" onclick="findNearby()" id="nearby-btn"
                    ...
                    <span id="nearby-btn-text">{{ $isNearby ? '✓ ' . __('Near Me') : __('Near Me') }}</span>
                </button>
    ...
    @if($isNearby)
        <div class="mb-6 p-4 rounded-xl bg-brand-50 border border-brand-100 text-brand-800 text-sm font-medium flex items-center gap-3">
            ...
            {{ __('Showing caregivers sorted by distance from your saved home location.') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:gap-8 gap-6">
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] overflow-hidden flex flex-col transition-all hover:-translate-y-1 hover:shadow-xl hover:border-brand-200">
                <div class="p-6 border-b border-slate-50 flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <a href="{{ route('family.employee.profile', $offer->employee->id) }}" class="group">
                            <h3 class="font-display font-bold text-lg text-slate-900 group-hover:text-brand-600 transition-colors truncate">{{ $offer->employee->user->name ?? __('Caregiver') }}</h3>
                        </a>
                ...
                <div class="p-6 flex-1 space-y-4">
                    <div class="flex items-center gap-3 text-sm font-medium text-slate-600">
                        ...
                        <span>{{ $offer->working_house ? __('Live-in') : __('Live-out') }} {{ __('Arrangement') }}</span>
                    </div>
                    <div class="flex items-center gap-3 text-sm font-medium text-slate-600">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                        <span>{{ __('Serves:') }} {{ $offer->commune }}, {{ $offer->wilaya }}</span>
                    </div>
                    <div class="pt-2">
                ...
                <div class="p-6 pt-0 mt-auto">
                    <button onclick="openBookingModal({{ $offer->id }}, '{{ addslashes($offer->employee->user->name ?? __('Caregiver')) }}', '{{ $offer->service_type }}')" 
        ...
        @empty
            <div class="col-span-1 md:col-span-2 lg:col-span-3 py-16 text-center w-full bg-white rounded-2xl border border-slate-100 shadow-sm">
                ...
                <h3 class="text-lg font-bold text-slate-800 mb-1">{{ __('No Caregivers Found') }}</h3>
                <p class="text-slate-500 font-medium text-sm">{{ __('Try adjusting your filters or expanding the search radius.') }}</p>
            </div>
        @endforelse
    </div>
        ...
        <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md transform scale-95 opacity-0 transition-all duration-300 flex flex-col overflow-hidden" id="modal-content">
            <form id="booking-form" method="POST" action="">
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                    <h3 class="font-display font-bold text-lg text-slate-800">{{ __('Request Caregiver') }}</h3>
                ...
                <div class="p-8 space-y-6">
                    <div class="bg-brand-50 border border-brand-100 rounded-2xl p-4 text-brand-800">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-brand-600 mb-1">{{ __('Booking Details') }}</p>
                        <p class="text-sm font-medium">{{ __('You are requesting') }} <strong id="modal-service" class="font-bold text-brand-900">...</strong> {{ __('from') }} <strong id="modal-name" class="font-bold text-brand-900">...</strong>.</p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">{{ __('Start Date') }} <span class="text-rose-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">{{ __('End Date') }} <span class="text-rose-500">*</span></label>
                    ...
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest text-center">{{ __('Dates are subject to caregiver approval.') }}</p>
                </div>
                <div class="px-8 py-5 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
                    <button type="button" onclick="closeBookingModal()" class="px-5 py-2.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl text-sm transition-all shadow-sm">{{ __('Cancel') }}</button>
                    <button type="submit" class="px-8 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-sm transition-all shadow-lg shadow-brand-500/30 active:scale-95">{{ __('Send Request') }}</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function findNearby() {
            ...
            @if($familyLoc && $familyLoc->latitude && $familyLoc->logitude)
                ...
                document.getElementById('filter-form').submit();
            @else
                alert("{{ __('Please set your home coordinates in Settings first to use Near Me.') }}");
            @endif
        }
    </script>
    @endpush
